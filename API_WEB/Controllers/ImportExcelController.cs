    ...
    public class ImportExcelAPIController : ControllerBase
    {
            ...
            [FromQuery] string? search, int pageSize = 0, int pageNumber = 1)
        {
            try
            {
                ...
                var TokenValidations = Models.Helpers.TokenValidation(HttpContext.Request.Headers["Bearer"]);
                ...
                {
                    string name = _userRepo.GetUserInfo(UserName).Name;
                    AccountList = await _accountDbService.GetAllAsync(t => t.Customer_ID == name);
                }
        ...
        public async Task<ActionResult<ViewModels.Models.APIResponse>> CreateAccount([FromBody] List<AccountModel> accountModel)
        {
            try
            {
                ...
                {
                    if (await _accountDbService.GetAsync(u => u.AccountNo == eachaccount.AccountNo && u.GL_CODE == eachaccount.GL_CODE && u.Customer_ID == u.Customer_ID) != null)
                    {
                        var model = await _accountDbService.GetAsync(u => u.AccountNo == eachaccount.AccountNo && u.GL_CODE == eachaccount.GL_CODE && u.Customer_ID == u.Customer_ID);
